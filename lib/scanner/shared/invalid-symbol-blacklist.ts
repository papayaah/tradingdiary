// Permanent database blacklist for invalid / non-existent ticker symbols (e.g. 404s).
...
function ensureTable(db: ReturnType<typeof postgres>): Promise<void> {
  if (!tableEnsured) {
    tableEnsured = db`
      CREATE TABLE IF NOT EXISTS invalid_symbol_blacklist (
...
/** Pre-load blacklisted symbols into memory for 0ms lookup performance. */
async function loadBlacklistCache(): Promise<void> {
  ...
  try {
    ...
    const rows = await db<Array<{ symbol: string }>>`
      SELECT symbol FROM invalid_symbol_blacklist
    `;
    for (const r of rows) {
...
/** Add a symbol to the permanent database blacklist. */
export async function blacklistSymbol(
  ...
  try {
    await ensureTable(db);
    await db`
      INSERT INTO invalid_symbol_blacklist (symbol, reason, provider, created_at)
...
/** Remove a symbol from the blacklist (e.g. if re-added or corrected). */
export async function removeFromBlacklist(symbol: string): Promise<void> {
  ...
  try {
    await ensureTable(db);
    await db`
      DELETE FROM invalid_symbol_blacklist WHERE symbol = ${cleanSymbol}
    `;
  } catch {
    // Best-effort
  }
...
export async function getBlacklistedSymbols(): Promise<BlacklistedSymbol[]> {
  const db = client();
  if (!db) {
