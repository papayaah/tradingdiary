...
- 🟡 **Steps 3–5 — scanner worker (shadow), core done and proven locally.** Under [lib/scanner/](../../lib/scanner/): `env.ts`, `db.ts` (dedicated pooled client — the app client is `max: 1`), `candles.ts` (reuses `getActiveProvider`, which falls back to server env keys; sanitize + 60-candle bound), `sessions.ts` (DST-correct ET eligibility), `queue.ts`/`scheduler.ts`/`worker.ts` (PostgreSQL-authoritative scheduling, deterministic one-shot jobs, at-least-once-safe handler, transactional state+alert+event+`pg_notify`, DB-unique dedup, shadow mode), `main.ts` (daemon: worker + scheduler tick + heartbeat + graceful shutdown). Run with `npm run scanner`. Verified end-to-end against throwaway Redis+Postgres (AAPL/Yahoo → state persisted, 0 alerts in shadow, event emitted). Unit tests for sessions and candle validation added.
  ...
  - **Remaining in 3–5:** add `redis` + `scanner` services to `docker-compose.yml`; startup reconciliation (rebuild due jobs from PG); per-provider rate limiter (Redis token bucket).
